...
public class ItemServiceImpl implements ItemService{
	...
	public ItemPage getItemPage(int pageNum) throws Exception {
		...
		//한페이지 당 아이템 5개씩 출력
		ItemPageSize itemPageSize = new ItemPageSize(size, (pageNum-1)*size);
		//limit sql문을 이용해 등록글 5개씩 가져온다
		List<ItemCommand> content = itemDao.selectItemList(itemPageSize);
		//총 등록글 수, 현재페이지, 한 페이지 당 등록글 수, 등록글을 넘겨준다(view)
...
//						itemImg.setImgUrl(PREFIX_IMG_URL + saveFileName);
	...
	//thumb외에 img 저장하는 메소드
		...
		//thumb 저장하는 메서드
		private void writeThumbFile(MultipartFile multipartFile, 
