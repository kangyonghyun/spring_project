...
public class ArticleController {
	...
	public String articleList(@ModelAttribute("article") Article article, @RequestParam(defaultValue="1") int curPage, Model model) throws Exception {
		...
		List<Article> byWriterIdArticleList = articleService.selectArticleByWriterId(article);
	...
	// 아이디로 리스트 페이지
	@RequestMapping(value = "/community/writerPage/{writerId}")
	public String byWriterIdList(@ModelAttribute("article") Article article,
			@RequestParam(defaultValue="1") int curPage,
			Model model) throws Exception {
		
		int listCnt = articleService.selectWriterPageCount(article);
		Pagination pagination = new Pagination(listCnt, curPage);
		article.setStartIndex(pagination.getStartIndex());
		article.setCntPerPage(pagination.getPageSize());
		
		List<Article> writerPage = articleService.selectArticleByWriterId(article);
		model.addAttribute("writerPage", writerPage);
		model.addAttribute("listCnt", listCnt);
		model.addAttribute("pagination", pagination);
		return "community/writerPage";
	}
...
//				model.addAttribute("url", url); //url들이 썸네일임
	...
	public void modArticleSubmit(@ModelAttribute("article") Article article, HttpSession session, HttpServletRequest request,
		...
		if (result > 0) {
			writer.println("<script>alert('수정 성공 - 파일 수정!'); location.href='" + request.getContextPath() + "/community/readArticle/"
					+ article.getArticleNo() + "';</script>");
		} else if(result==0){
			writer.println("<script>alert('수정 성공 - 파일 수정 없음!'); location.href='" + request.getContextPath() + "/community/readArticle/"
					+ article.getArticleNo() + "';</script>");
